...
class scroller(tk.Frame):
    ...
    def frame_deploy(self):
        self.mainframe = tk.Frame(self, highlightthickness=0, background="yellow")
        self.mainframe.pack(fill=tk.BOTH, side=tk.LEFT, expand=1, anchor=tk.NW)
        self.canvas = tk.Canvas(self.mainframe, highlightthickness=0, background='red')
    ...
    def update_scroll_region(self, *event):
        ...
        if frame_height <= self.canvas.winfo_height():
...
test_frame = tk.Frame(root, highlightthickness=0, background="black")
...
for i in range(100):
    label = tk.Label(adm_T_C_scroller.frame, text=f"Label {i + 1}", width=10, background="blue")
    label.pack(side=tk.TOP, fill=tk.Y, expand=1)
